...
def test_create_vector_store_env_override(monkeypatch):
    """VECTOR_STORE_BACKEND 环境变量优先于 config；默认 milvus（2026-08-27 产品切 Milvus）。"""
    from app.rag.vector_store import VectorStore, create_vector_store

    # 默认（无 env）：返回 MilvusVectorStore（config 默认 milvus；Milvus 不可达时整模块 skip）
    ...
    assert vs.backend == "milvus"
    assert isinstance(vs, MilvusVectorStore)

    # env=chroma：显式切回 Chroma
    monkeypatch.setenv("VECTOR_STORE_BACKEND", "chroma")
    monkeypatch.delenv("E2E_COLLECTION", raising=False)
    vs2 = create_vector_store(collection_name="kb_env_test")
    assert vs2.backend == "chroma"
    assert isinstance(vs2, VectorStore)

    # env=milvus：milvus + E2E_COLLECTION 覆盖生效
    monkeypatch.setenv("VECTOR_STORE_BACKEND", "milvus")
    monkeypatch.setenv("E2E_COLLECTION", "crud_rag")
    vs3 = create_vector_store(collection_name="kb_ignored")
    assert vs3.backend == "milvus"
    assert isinstance(vs3, MilvusVectorStore)
    assert vs3.collection_name == "crud_rag"  # E2E_COLLECTION 覆盖生效
