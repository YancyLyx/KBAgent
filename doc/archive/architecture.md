...
用户提问 → Chat API → AgentManager
  │
  ├── ReAct 循环（轮数上限 20，仅作极端兜底）：
  │   1. LLM 推理：当前 context + 工具列表 → 决策
  │   2. 如果 LLM 调用工具 → 执行 → 观察结果 → 追加到 context → 返回步骤 1
  │   3. 如果 LLM 直接回答 → reason=completed → 返回回答
  │      异常终止：无进展止损 / 指纹防循环 / 输出超长 / 上下文超长
  │
...
模型单次输出被 `max_tokens` 截断（`finish_reason=length`）时，不直接给残缺
答案，而是**保留已生成内容**（累积到最终回答）+ 在下一轮消息里追加提示让
模型从断点续写：

```text
（系统提示：上次输出因长度限制被截断。请从断点直接续写，不要重复或总结
 已输出内容，不要道歉，把剩余内容拆成更小的块继续输出。）
```

计数上限：`output_recovery_max=3`（`loop_config.yaml`），超过则退出
`reason=output_truncated_max` 并返回已累积内容——nudge 不能无限循环，
否则陷入"截断→续写→再截断"死锁（Claude maxOutputTokensRecovery 同款）。

### 被动上下文压缩

与演进式摘要（主动压缩）互补：这是**API 拒收后的被动补救**。当模型调用抛
"context length / too many tokens" 类错误时：

1. 同一轮内**只压一次**（`attempted_compact` 防重复标志，防止
   "压缩→仍超长→再压缩"死循环）
2. 压缩规则：保留全部 system 消息 + 最近 4 条非 system 消息
3. 重试一次；仍失败退出 `reason=prompt_too_long`

为什么被动触发而不是主动压：压缩本身要烧 token 调模型，能不压就不压——
先按原样发，被拒了再补救（Claude hasAttemptedReactiveCompact 同款）。

## 兜底策略（四层框架）

检索不到不等于没有答案。用户提问口语化导致的"假性失败"，按四层递进处理，
每层解决一个不同维度的问题，缺一不可（参考第二篇面经的四层框架）：

### 1. 检索层抢救（先别急着说"没找到"）

**解决什么**：大部分"检索不到"其实是没命中，不是库里没有。

| 手段 | 实现 |
|---|---|
| 查询改写 | 多查询分解 + HyDE（拆不拆由 LLM 判断）；指代补全（"那个方案"→实体，与闲聊判断合并成一次意图分析） |
| 混合检索 | 向量（bge）+ BM25（bigram）→ RRF，语义和精确词互补 |
| 多召回候选 | top_k×3 放大再精排；空结果时用**原始问题**（而非 LLM 工具参数）再召一次低相关候选 |

### 2. 生成层边界（宁可说不知道，也不编）

**解决什么**：合规场景编造一句的代价远大于"不知道"。

- 诚实原则写进 system prompt："检索结果提示未找到，明确告知，不要编造答案"
- **strict/lenient 场景标签**（`fallback_policy.strict_tags`）：薪酬/股权/报销等
  制度类默认 strict——检索不到明确拒绝，不用模型知识猜；lenient 允许通用知识
  兜底但必须标注"以下为通用知识，并非企业知识库内容"

### 3. 交互层挽回（把"没找到"变成一次挽回体验）

**解决什么**：直接返回"未找到"用户流失率高。

- 空结果附引导话术："未找到完全匹配的内容，可换关键词或补充细节再试"
- 不精排再召 top-3 附"以下资料相关度较低，仅供参考"——把决定权交给用户

### 4. 系统层闭环（失败 query 反哺知识库）

**解决什么**：让系统越用越好，而不是每次失败都从零开始。

- `data/missed_queries.jsonl` 记录检索失败的**原始问题**（盲区分析面向"用户怎么问"）
- 高频聚合 → 管理端 `/api/admin/eval/missed` 展示知识库盲区
- 反向：低频兜底答案不写缓存、badcase 可回溯（memory_context 埋点）

**工程取舍**（第二篇的六个决策点）：查询改写最多一次（每改写延迟翻倍）；
RRF 固定权重起步；候选提供按场景分档；失败 query 离线聚合为主、实时只做
高频异常告警——详见 interview_prep 3.5 的"兜底侧工程取舍"表。
...
- 扩展方向：延迟敏感场景的精排降级开关
  （评测显示精排对回复质量提升有限但延迟 15 倍；当前默认开保 HR@1，
  降级开关留给高延迟场景）、模型自托管（vLLM，OpenAI 兼容协议无缝切换）
